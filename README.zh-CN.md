...
当前版本（v1.4.x）做捕获、打标、切分、本地存储、剪贴板，以及按需的 ASR 分析报告（SiliconFlow SenseVoiceSmall，自备 API Key，在录音库窗口设置）。暂不包含：飞书 API 同步（接口已在 `FeishuSyncService.swift` 预留；报告本身就是为外部 CLI 上传设计的）、回放/复习 UI、间隔复习计划、Anki 导出。得益于结构化的本地数据（会话元数据里的绿段时间区间 + 总索引），这些都可以在不改动捕获流水线的前提下往上叠。
