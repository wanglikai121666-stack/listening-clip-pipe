...
Listening Clip Pipe 是一个为**雅思听力训练**（以及一切认真的语言听力学习）设计的 macOS 菜单栏工具。听到连读、弱读、吞音听不懂时，按 **⌥Z** 开始录制系统音频，再按 **⌥Z** 停止——片段自动保存到本地并写入剪贴板，切到飞书 / Notion / 任意文档 **Cmd+V**，音频就落在你正在写笔记的位置。

**当前为 1.0 版本**——完整的「捕获 → 保存 → 粘贴」流水线，刻意保持极简。
...
- **⚡ 一个快捷键，零切换成本。** ⌥Z 开始、⌥Z 结束，全程不离开播放器和笔记。
- **🔴 录音状态绝不会看漏。** 录制期间屏幕右上角悬浮一个置顶的 ● REC 计时器——全屏 App 之上也可见，鼠标事件穿透，不挡任何操作。
...
1. 播放雅思听力 / 播客 / 课程音频。
2. 听到听不懂的地方 → 按 **⌥Z**（菜单栏图标变红，屏幕右上角出现悬浮 ● REC 计时器）。
3. 这一小段放完 → 再按 **⌥Z**。
4. 收到通知 *Clip copied*，切到笔记 **Cmd+V**。
5. 在音频旁边写下听错了什么、为什么听错。

菜单栏菜单还提供：**Copy Last Clip + Anchor**、**Copy Last Clip**、**Copy Last Anchor**（粘贴失败时补救用）、**Open Clips Folder**。
